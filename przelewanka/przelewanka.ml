...
let przelewanka input_arr = 
  ...
  (* kazda wielkosc tablicy docelowej dzieli sie przez nwd pojemnosci szklanek
  ...
  else if not (Array.for_all (fun (_, y) -> y mod common_div = 0) input_arr)
    ...
    -1
      ...
      (* Wykonujemy BFS. Z wlasnosci tego przeszukiwania wiemy,ze gdy 
         znajdziemy poszukiwany wynik bedzie osiagalny optymalnym kosztem. *)
